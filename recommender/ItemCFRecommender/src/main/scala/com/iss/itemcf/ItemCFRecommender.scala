...
object ItemCFRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // TODO: 核心算法，计算同现相似度，得到书籍的相似列表
    // 统计每个书籍的评分个数，按照productId来做group by
    ...
    // 将评分按照用户id两两配对，统计两个书籍被同一个用户评分过的次数
    ...
    // 按照product1,product2 做group by，统计userId的数量，就是对两个书籍同时评分的人数
